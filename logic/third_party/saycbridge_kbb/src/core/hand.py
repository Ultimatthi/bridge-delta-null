...
class Hand(object):
    def __init__(self, cards_by_suit):
        ...
        self.cards_by_suit = list(map(hand_sorter, list(map(str.upper, cards_by_suit))))
        self._validate()
    ...
    def _validate(self):
        assert sum([self.length_of_suit(suit) for suit in SUITS]) == 13, self.cards_by_suit
    ...
    def sorted_cards(self):
        ...
        return sum([[Card(suit, value_char) for value_char in sorted_cards_by_suit[suit]] for suit in sorted_suits], [])
    ...
    def suit_lengths(self):
        return list(map(len, self.cards_by_suit))

    def longest_suits(self):
        longest_suit_length = max(self.suit_lengths())
        return [suit for suit in SUITS if len(self.cards_by_suit[suit]) == longest_suit_length]
