...
class VulnerabilityTest(unittest2.TestCase):
    def test_vunerability_from_board_number(self):
        ...
        for number, expected_vulnerability in list(expectations.items()):
            self.assertEqual(Vulnerability.from_board_number(number).name, expected_vulnerability)


class CallHistoryTest(unittest2.TestCase):

    def _assert_declarer(self, history_string, dealer, declarer):
        self.assertEqual(CallHistory.from_string(history_string, dealer.char).declarer(), declarer)
    ...
    def test_position_to_call(self):
        ...
        self.assertEqual(CallHistory.from_string("1N P").position_to_call(), SOUTH)
        self.assertEqual(CallHistory.from_string("1N P 2C").position_to_call(), WEST)
        self.assertEqual(CallHistory.from_string("1N P 2C P").position_to_call(), NORTH)
        self.assertEqual(CallHistory.from_string("1N P 2C P 2D").position_to_call(), EAST)
    ...
    def test_last_to_call(self):
        ...
        self.assertEqual(CallHistory.from_string("1N P 2C").last_to_call, SOUTH)
        self.assertEqual(CallHistory.from_string("1N P 2C P").last_to_call, WEST)
        self.assertEqual(CallHistory.from_string("1N P 2C P 2D").last_to_call, NORTH)
    ...
    def test_is_passout(self):
        ...
        self.assertEqual(CallHistory.from_string("P 1N P P P").is_passout(), False)
    ...
    def _assert_competative_auction(self, history_string, is_competative):
        self.assertEqual(CallHistory.from_string(history_string).competative_auction(), is_competative)
    ...
    def _assert_opener(self, history_string, dealer, opener):
        self.assertEqual(CallHistory.from_string(history_string, dealer.char).opener(), opener)
    ...
    def _assert_is_legal_call(self, history_string, bid, is_legal=True):
        ...
        self.assertEqual(call_history.is_legal_call(bid), is_legal)
