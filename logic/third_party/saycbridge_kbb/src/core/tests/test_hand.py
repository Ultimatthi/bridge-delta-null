...
class HandTest(unittest2.TestCase):
    def _assert_balanced(self, hand_string, balanced):
        self.assertEqual(Hand.from_cdhs_string(hand_string).is_balanced(), balanced)
    ...
    def _assert_non_working_honor_adjustment(self, hand_string, trump, expected_adjustment):
        ...
        self.assertEqual(hand._support_point_adjustment_for_non_working_honors(trump), expected_adjustment)
    ...
    def test_support_points(self):
        self.assertEqual(Hand.from_cdhs_string("AKJ52.J.J9743.54").support_points(HEARTS), 13)
        self.assertEqual(Hand.from_cdhs_string("AKJ52.J.J9743.54").generic_support_points(), 13)
        self.assertEqual(Hand.from_cdhs_string("AKJ52..J9743.J54").generic_support_points(), 15)
    ...
    def _assert_stopper(self, cards_string, expected_round):
        ...
        self.assertEqual(hand.has_fourth_round_stopper(CLUBS), bool(expected_round) and expected_round <= 4)
        self.assertEqual(hand.has_third_round_stopper(CLUBS), bool(expected_round) and expected_round <= 3)
        self.assertEqual(hand.has_second_round_stopper(CLUBS), bool(expected_round) and expected_round <= 2)
        self.assertEqual(hand.has_first_round_stopper(CLUBS), bool(expected_round) and expected_round <= 1)
