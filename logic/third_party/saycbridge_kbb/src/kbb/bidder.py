...
class ConsistencyOracle(object):
    ...
    def _trump_suit(self, knowledge):
        suit_fits = [suit for suit in SUITS if (knowledge.me.min_length(suit) + knowledge.partner.min_length(suit)) >= 8]
        if len(suit_fits) == 1:
            return suit_fits[0]
...
class KnowledgeBasedBidder(object):
    ...
    def call_selection_for(self, hand, history):
        ...
        all_priorities, _, _, _ = list(zip(*prioritiy_bid_rule_hand_knowledge_tuples))
