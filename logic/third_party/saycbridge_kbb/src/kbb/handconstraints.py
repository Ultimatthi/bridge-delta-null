...
class HonorConstraint(object):
    UNKNOWN, FOURTH_ROUND_STOPPER, THIRD_ROUND_STOPPER, THREE_OF_TOP_FIVE, TWO_OF_TOP_THREE = list(range(5))
...
class ControlConstraint(object):
    FIRST_ROUND, SECOND_ROUND = list(range(2))
...
class HandConstraints(object):
    ...
    def _compute_implied_suit_length_ranges(self):
        ...
        suit_minimum_lengths = [range_tuple[0] for range_tuple in self._suit_length_ranges]
        suit_maximum_lengths = [range_tuple[1] for range_tuple in self._suit_length_ranges]
        for suit in SUITS:
    ...
    def pretty_one_line(self):
        ...
        suit_strings = [string for string in suit_strings if bool(string)]
