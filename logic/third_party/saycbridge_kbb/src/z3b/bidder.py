...
class Bidder(object):
    ...
    def call_selection_for(self, hand, call_history, expected_call=None):
        with Interpreter().create_history(call_history) as history:
            ...
            maximal_calls = [call for call in maximal_calls if not rule_selector.rule_for_call(call).requires_planning]
            ...
            if len(maximal_calls) != 1:
                ...
                print("WARNING: Unordered: %s from: %s (%s)" % (call_names, rules, list(map(possible_calls.priority_for_call, maximal_calls))))
                return None
